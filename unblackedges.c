...
void read_pbm(FILE *input, Bit2 *pbm_array) {
    for (int i = 0; i < pbm_array->rows; i++) {
        for (int j = 0; j < pbm_array->columns; j++) {
            ...
            if (value == BLACK) {
                Bit_set(pbm_array->Bit_Array, i * pbm_array->columns + j, BLACK);
            }
        }
    }
}
...
void process_edges(Bit2 *pbm_array) {
    ...
    for (int j = 0; j < columns; j++) {
        if (Bit_get(pbm_array->Bit_Array, 0 * columns + j) == BLACK) {
            flood_fill(pbm_array, 0, j);
        }
        if (Bit_get(pbm_array->Bit_Array, (rows - 1) * columns + j) == BLACK) {
            flood_fill(pbm_array, rows - 1, j);
        }
    }

    for (int i = 0; i < rows; i++) {
        if (Bit_get(pbm_array->Bit_Array, i * columns + 0) == BLACK) {
            flood_fill(pbm_array, i, 0);
        }
        if (Bit_get(pbm_array->Bit_Array, i * columns + (columns - 1)) == BLACK) {
...
void flood_fill(Bit2 *pbm_array, int row, int col) {
    ...
    if (row < 0 || row >= rows || col < 0 || col >= columns || 
        Bit_get(pbm_array->Bit_Array, row * columns + col) == WHITE) {
        return;
    }

    Bit_set(pbm_array->Bit_Array, row * columns + col, WHITE);
